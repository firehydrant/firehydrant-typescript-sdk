...
async function run() {
  const result = await firehydrantTypescriptSDK.system.ping();
...
import { systemPing } from "firehydrant-typescript-sdk/funcs/systemPing.js";
...
async function run() {
  const res = await systemPing(firehydrantTypescriptSDK);
