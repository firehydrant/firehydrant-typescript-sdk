...
async function run() {
  const result = await firehydrantTypescriptSDK.tasks.list.delete({
    taskListId: "<id>",
  });
...
import { tasksListDelete } from "firehydrant-typescript-sdk/funcs/tasksListDelete.js";
...
async function run() {
  const res = await tasksListDelete(firehydrantTypescriptSDK, {
    taskListId: "<id>",
  });
