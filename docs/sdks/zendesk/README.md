...
async function run() {
  await firehydrantTypescriptSDK.zendesk.searchTickets({
    ticketId: "<id>",
  });
...
import { zendeskSearchTickets } from "firehydrant-typescript-sdk/funcs/zendeskSearchTickets.js";
...
async function run() {
  const res = await zendeskSearchTickets(firehydrantTypescriptSDK, {
    ticketId: "<id>",
  });
