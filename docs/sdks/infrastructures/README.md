...
async function run() {
  const result = await firehydrantTypescriptSDK.infrastructures.list({});
...
import { infrastructuresList } from "firehydrant-typescript-sdk/funcs/infrastructuresList.js";
...
async function run() {
  const res = await infrastructuresList(firehydrantTypescriptSDK, {});
