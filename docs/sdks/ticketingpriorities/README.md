...
async function run() {
  const result = await firehydrantTypescriptSDK.ticketingPriorities.get({
    id: "<id>",
  });
...
import { ticketingPrioritiesGet } from "firehydrant-typescript-sdk/funcs/ticketingPrioritiesGet.js";
...
async function run() {
  const res = await ticketingPrioritiesGet(firehydrantTypescriptSDK, {
    id: "<id>",
  });
