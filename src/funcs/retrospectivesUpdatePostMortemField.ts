 ...
 */
export function retrospectivesUpdatePostMortemField(
  client: FirehydrantCore,
  request: operations.UpdatePostMortemFieldRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.PostMortemsSectionFieldEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdatePostMortemFieldRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_post_mortem_field, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PostMortemsSectionFieldEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
