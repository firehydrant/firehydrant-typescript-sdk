 ...
 */
export function incidentsListIncidentEvents(
  client: FirehydrantCore,
  request: operations.ListIncidentEventsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.IncidentEventEntityPaginated,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListIncidentEventsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/events")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.IncidentEventEntityPaginated$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
