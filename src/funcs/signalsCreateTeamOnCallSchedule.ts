...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateTeamOnCallScheduleRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.create_team_on_call_schedule, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/teams/{team_id}/on_call_schedules")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.SignalsAPIOnCallScheduleEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
