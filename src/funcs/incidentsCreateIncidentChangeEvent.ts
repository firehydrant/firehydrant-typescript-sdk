 ...
 */
export function incidentsCreateIncidentChangeEvent(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateIncidentChangeEventRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.create_incident_change_event, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/related_change_events")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.IncidentsRelatedChangeEventEntity$inboundSchema),
