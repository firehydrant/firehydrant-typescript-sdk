 ...
 */
export function incidentsUpdateScheduledMaintenance(
  client: FirehydrantCore,
  request: operations.UpdateScheduledMaintenanceRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ScheduledMaintenanceEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateScheduledMaintenanceRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_scheduled_maintenance, {
    explode: true,
  });

  const pathParams = {
    scheduled_maintenance_id: encodeSimple(
      "scheduled_maintenance_id",
      payload.scheduled_maintenance_id,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ScheduledMaintenanceEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
