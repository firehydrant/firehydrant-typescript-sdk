 ...
 */
export function metricsReportingGetSignalsNoiseAnalytics(
  client: FirehydrantCore,
  request: operations.GetSignalsNoiseAnalyticsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SignalsAPIAnalyticsNoiseMetricsEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetSignalsNoiseAnalyticsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/signals/analytics/noise/metrics")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SignalsAPIAnalyticsNoiseMetricsEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
