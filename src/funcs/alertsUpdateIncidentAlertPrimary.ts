 ...
 */
export function alertsUpdateIncidentAlertPrimary(
  client: FirehydrantCore,
  request: operations.UpdateIncidentAlertPrimaryRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.IncidentsAlertEntity,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateIncidentAlertPrimaryRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_incident_alert_primary, {
    explode: true,
  });

  const pathParams = {
    incident_alert_id: encodeSimple(
      "incident_alert_id",
      payload.incident_alert_id,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.IncidentsAlertEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
