 ...
 */
export function signalsCopyOnCallScheduleRotation(
  client: FirehydrantCore,
  request: operations.CopyOnCallScheduleRotationRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SignalsAPIOnCallRotationEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CopyOnCallScheduleRotationRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.copy_on_call_schedule_rotation, {
    explode: true,
  });

  const pathParams = {
    rotation_id: encodeSimple("rotation_id", payload.rotation_id, {
      explode: false,
  ...
  const path = pathToFunc(
    "/v1/teams/{team_id}/on_call_schedules/{schedule_id}/rotations/{rotation_id}/copy",
  )(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.SignalsAPIOnCallRotationEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
