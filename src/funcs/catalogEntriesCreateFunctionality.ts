 ...
 */
export function catalogEntriesCreateFunctionality(
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CreateFunctionality$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/functionalities")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.FunctionalityEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
