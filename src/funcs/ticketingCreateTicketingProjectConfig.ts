 ...
 */
export function ticketingCreateTicketingProjectConfig(
  client: FirehydrantCore,
  request: operations.CreateTicketingProjectConfigRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.TicketingProjectConfigEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateTicketingProjectConfigRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.TicketingProjectConfigEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
