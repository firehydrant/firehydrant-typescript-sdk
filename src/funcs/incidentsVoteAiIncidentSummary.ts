 ...
 */
export function incidentsVoteAiIncidentSummary(
  client: FirehydrantCore,
  request: operations.VoteAiIncidentSummaryRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AIEntitiesIncidentSummaryEntity,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.VoteAiIncidentSummaryRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    generated_summary_id: encodeSimple(
      "generated_summary_id",
      payload.generated_summary_id,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.AIEntitiesIncidentSummaryEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
