 ...
 * Add impacted infrastructure to an incident
 */
export async function incidentsCreateImpact(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateIncidentImpactRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.postV1IncidentsIncidentIdImpactType, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/impact/{type}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.IncidentImpactEntity$inboundSchema),
