 ...
 */
export function signalsListTeamSignalRules(
  client: FirehydrantCore,
  request: operations.ListTeamSignalRulesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SignalsAPIRuleEntityPaginated,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListTeamSignalRulesRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SignalsAPIRuleEntityPaginated$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
