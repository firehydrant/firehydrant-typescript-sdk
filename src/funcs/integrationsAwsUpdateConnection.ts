 ...
 */
export async function integrationsAwsUpdateConnection(
  client: FirehydrantTypescriptSDKCore,
  request: operations.UpdateAwsConnectionRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.IntegrationsAwsConnectionEntity,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateAwsConnectionRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.patchV1IntegrationsAwsConnectionsId, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/integrations/aws/connections/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.IntegrationsAwsConnectionEntity$inboundSchema),
    M.jsonErr(
