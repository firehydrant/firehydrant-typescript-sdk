 ...
 */
export async function incidentSettingsListIncidentTypes(
  client: FirehydrantTypescriptSDKCore,
  request: operations.ListIncidentTypesRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.IncidentTypeEntityPaginated,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListIncidentTypesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/incident_types")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.IncidentTypeEntityPaginated$inboundSchema),
    M.jsonErr(
