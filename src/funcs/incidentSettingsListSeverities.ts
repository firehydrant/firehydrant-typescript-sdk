 ...
 * Lists severities
 */
export async function incidentSettingsListSeverities(
  client: FirehydrantTypescriptSDKCore,
  request: operations.ListSeveritiesRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.SeverityEntityPaginated,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListSeveritiesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/severities")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.SeverityEntityPaginated$inboundSchema),
    M.jsonErr(
