 ...
 */
export function integrationsRefreshConnection(
  client: FirehydrantCore,
  request: operations.RefreshConnectionRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.IntegrationsConnectionEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.RefreshConnectionRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.IntegrationsConnectionEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
