 ...
 */
export function catalogEntriesGetServiceDependencies(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetServiceDependenciesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/services/{service_id}/dependencies")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ServiceWithAllDependenciesEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
