 ...
 */
export function catalogEntriesListServices(
  client: FirehydrantCore,
  request: operations.ListServicesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ServiceEntityPaginated,
    | FirehydrantError
...
> {
  ...
  const path = pathToFunc("/v1/services")();
  ...
  const query = queryJoin(
    ...
    encodeFormQuery({
      "available_downstream_dependencies_for_id":
        payload.available_downstream_dependencies_for_id,
      "available_upstream_dependencies_for_id":
        payload.available_upstream_dependencies_for_id,
      ...
      "query": payload.query,
      "responding_teams": payload.responding_teams,
      "tiers": payload.tiers,
    }),
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ServiceEntityPaginated$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
