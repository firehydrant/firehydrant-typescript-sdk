 ...
 */
export async function incidentSettingsUpdatePriority(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdatePriorityRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.patchV1PrioritiesPrioritySlug, {
    explode: true,
  });

  const pathParams = {
    priority_slug: encodeSimple("priority_slug", payload.priority_slug, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v1/priorities/{priority_slug}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PriorityEntity$inboundSchema),
    M.jsonErr(
