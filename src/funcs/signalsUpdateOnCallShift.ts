 ...
 */
export function signalsUpdateOnCallShift(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateOnCallShiftRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_on_call_shift, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SignalsAPIOnCallShiftEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
