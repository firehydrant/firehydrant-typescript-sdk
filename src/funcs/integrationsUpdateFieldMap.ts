...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateIntegrationFieldMapRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    field_map_id: encodeSimple("field_map_id", payload.field_map_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v1/integrations/field_maps/{field_map_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.FieldMappingFieldMapEntity$inboundSchema),
    M.jsonErr(
