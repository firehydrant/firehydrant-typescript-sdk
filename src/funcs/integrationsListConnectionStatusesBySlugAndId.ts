 ...
 */
export function integrationsListConnectionStatusesBySlugAndId(
  client: FirehydrantCore,
  request: operations.ListConnectionStatusesBySlugAndIdRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.IntegrationsConnectionStatusEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListConnectionStatusesBySlugAndIdRequest$outboundSchema.parse(
        value,
      ),
  ...
  const pathParams = {
    by_connection_id: encodeSimple(
      "by_connection_id",
      payload.by_connection_id,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.IntegrationsConnectionStatusEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
