...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateRunbookRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_runbook, { explode: true });
  ...
  const path = pathToFunc("/v1/runbooks/{runbook_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.RunbookEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
