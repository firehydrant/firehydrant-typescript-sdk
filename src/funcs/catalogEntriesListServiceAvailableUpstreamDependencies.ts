 ...
 */
export function catalogEntriesListServiceAvailableUpstreamDependencies(
  client: FirehydrantCore,
  request: operations.ListServiceAvailableUpstreamDependenciesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ServiceEntityLite,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListServiceAvailableUpstreamDependenciesRequest$outboundSchema
        .parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ServiceEntityLite$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
