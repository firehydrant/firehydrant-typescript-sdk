 ...
 */
export function signalsOverrideOnCallScheduleRotationShifts(
  client: FirehydrantCore,
  request: operations.OverrideOnCallScheduleRotationShiftsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SignalsAPIOnCallShiftEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.OverrideOnCallScheduleRotationShiftsRequest$outboundSchema
        .parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload.override_on_call_schedule_rotation_shifts,
    { explode: true },
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.SignalsAPIOnCallShiftEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
