...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateChecklistTemplateRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.patchV1ChecklistTemplatesId, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/checklist_templates/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ChecklistTemplateEntity$inboundSchema),
    M.jsonErr(
