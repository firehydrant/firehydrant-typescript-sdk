 ...
 * Unarchive an incident
 */
export function incidentsUnarchiveIncident(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UnarchiveIncidentRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/unarchive")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.IncidentEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
