...
> {
  const parsed = safeParse(
    request,
    (value) => operations.IngestCatalogDataRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.postV1CatalogsCatalogIdIngest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/catalogs/{catalog_id}/ingest")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.ImportsImportEntity$inboundSchema),
    M.jsonErr(
