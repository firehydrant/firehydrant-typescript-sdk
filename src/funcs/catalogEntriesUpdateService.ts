 ...
 */
export function catalogEntriesUpdateService(
  client: FirehydrantCore,
  request: operations.UpdateServiceRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ServiceEntity,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateServiceRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_service, { explode: true });
  ...
  const path = pathToFunc("/v1/services/{service_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ServiceEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
