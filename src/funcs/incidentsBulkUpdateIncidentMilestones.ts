 ...
 */
export function incidentsBulkUpdateIncidentMilestones(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.BulkUpdateIncidentMilestonesRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.bulk_update_incident_milestones, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/milestones/bulk_update")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.IncidentsMilestoneEntityPaginated$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
