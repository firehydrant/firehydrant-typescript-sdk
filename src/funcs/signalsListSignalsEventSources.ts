 ...
 */
export function signalsListSignalsEventSources(
  client: FirehydrantCore,
  request: operations.ListSignalsEventSourcesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SignalsAPITransposerListEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListSignalsEventSourcesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/signals/event_sources")();

  const query = encodeFormQuery({
    "escalation_policy_id": payload.escalation_policy_id,
    "on_call_schedule_id": payload.on_call_schedule_id,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SignalsAPITransposerListEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
