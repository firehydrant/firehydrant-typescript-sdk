 ...
 */
export function retrospectivesUpdateIncidentRetrospective(
  client: FirehydrantCore,
  request: operations.UpdateIncidentRetrospectiveRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.IncidentsRetrospectiveEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateIncidentRetrospectiveRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_incident_retrospective, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.IncidentsRetrospectiveEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
