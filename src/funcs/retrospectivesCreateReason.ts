 ...
 */
export async function retrospectivesCreateReason(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateRetrospectiveReportReasonRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload.postV1PostMortemsReportsReportIdReasons,
    { explode: true },
  );
  ...
  const path = pathToFunc("/v1/post_mortems/reports/{report_id}/reasons")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.PostMortemsReasonEntity$inboundSchema),
    M.jsonErr(
