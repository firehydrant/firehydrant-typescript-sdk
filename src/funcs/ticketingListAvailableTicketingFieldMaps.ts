 ...
 */
export function ticketingListAvailableTicketingFieldMaps(
  client: FirehydrantCore,
  request: operations.ListAvailableTicketingFieldMapsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.TicketingFieldMapsMappableFieldEntity,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAvailableTicketingFieldMapsRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.TicketingFieldMapsMappableFieldEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
