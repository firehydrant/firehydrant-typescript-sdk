 ...
 */
export async function incidentsDeleteTeamAssignment(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.DeleteIncidentTeamAssignmentRequest$outboundSchema.parse(
        value,
      ),
  ...
  if (payload.RequestBody != null) {
    if (payload.RequestBody.role_assignment_ids !== undefined) {
      body.append(
        "role_assignment_ids",
        String(payload.RequestBody.role_assignment_ids),
      );
    }
  }
  ...
  const pathParams = {
    ...
    team_assignment_id: encodeSimple(
      "team_assignment_id",
      payload.team_assignment_id,
