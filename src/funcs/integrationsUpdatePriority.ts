...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateTicketingPriorityRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.patchV1TicketingPrioritiesId, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/ticketing/priorities/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.TicketingPriorityEntity$inboundSchema),
    M.jsonErr(
