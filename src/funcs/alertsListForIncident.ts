 ...
 */
export async function alertsListForIncident(
  client: FirehydrantTypescriptSDKCore,
  request: operations.ListIncidentAlertsRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.IncidentsAlertEntityPaginated,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListIncidentAlertsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/alerts")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.IncidentsAlertEntityPaginated$inboundSchema),
    M.jsonErr(
