 ...
 */
export async function incidentsCreateTaskList(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateIncidentTaskListRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.postV1IncidentsIncidentIdTaskLists, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/task_lists")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.TaskEntity$inboundSchema),
    M.jsonErr(
