 ...
 */
export async function metricsGetInfrastructure(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetInfrastructureMetricsRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    infra_id: encodeSimple("infra_id", payload.infra_id, {
      explode: false,
      charEncoding: "percent",
    }),
    infra_type: encodeSimple("infra_type", payload.infra_type, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v1/metrics/{infra_type}/{infra_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.MetricsInfrastructureMetricsEntity$inboundSchema),
    M.jsonErr(
