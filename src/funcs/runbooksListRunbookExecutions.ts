 ...
 */
export function runbooksListRunbookExecutions(
  client: FirehydrantCore,
  request: operations.ListRunbookExecutionsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.RunbooksExecutionEntityPaginated,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListRunbookExecutionsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/runbooks/executions")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.RunbooksExecutionEntityPaginated$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
