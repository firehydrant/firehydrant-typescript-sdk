 ...
 */
export function integrationsUpdateAwsCloudtrailBatch(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateAwsCloudtrailBatchRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_aws_cloudtrail_batch, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/integrations/aws/cloudtrail_batches/{id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.IntegrationsAwsCloudtrailBatchEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
