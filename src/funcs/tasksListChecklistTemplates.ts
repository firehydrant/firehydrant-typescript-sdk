 ...
 */
export async function tasksListChecklistTemplates(
  client: FirehydrantTypescriptSDKCore,
  request: operations.ListChecklistTemplatesRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.ChecklistTemplateEntityPaginated,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListChecklistTemplatesRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ChecklistTemplateEntityPaginated$inboundSchema),
    M.jsonErr(
