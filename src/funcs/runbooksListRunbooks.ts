 ...
 */
export function runbooksListRunbooks(
  client: FirehydrantCore,
  request: operations.ListRunbooksRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SlimRunbookEntityPaginated,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListRunbooksRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/runbooks")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SlimRunbookEntityPaginated$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
