 ...
 */
export function tasksUpdateIncidentTask(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateIncidentTaskRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_incident_task, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/tasks/{task_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.TaskEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
