 ...
 */
export function integrationsUpdateAuthedProvider(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateAuthedProviderRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    authed_provider_id: encodeSimple(
      "authed_provider_id",
      payload.authed_provider_id,
    ...
    integration_slug: encodeSimple(
      "integration_slug",
      payload.integration_slug,
      { explode: false, charEncoding: "percent" },
    ),
  };
  ...
  >(
    M.json(
      200,
      components.PublicApiv1IntegrationsAuthedProviderEntity$inboundSchema,
    ),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
