 ...
 */
export function retrospectivesDeleteRetrospectiveTemplate(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.DeleteRetrospectiveTemplateRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    retrospective_template_id: encodeSimple(
      "retrospective_template_id",
      payload.retrospective_template_id,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.RetrospectivesTemplateEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
