 ...
 */
export function catalogEntriesGetEnvironment(
  client: FirehydrantCore,
  request: operations.GetEnvironmentRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.EnvironmentEntryEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetEnvironmentRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/environments/{environment_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.EnvironmentEntryEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
