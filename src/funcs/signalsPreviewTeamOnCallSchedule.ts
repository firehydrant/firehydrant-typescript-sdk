 ...
 */
export function signalsPreviewTeamOnCallSchedule(
  client: FirehydrantCore,
  request: operations.PreviewTeamOnCallScheduleRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SignalsAPIOnCallSchedulePreviewEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PreviewTeamOnCallScheduleRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.preview_team_on_call_schedule, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/teams/{team_id}/on_call_schedules/preview")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.SignalsAPIOnCallSchedulePreviewEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
