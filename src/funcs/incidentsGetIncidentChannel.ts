 ...
 */
export function incidentsGetIncidentChannel(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetIncidentChannelRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/channel")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.IncidentsChannelEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
