...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateRunbookExecutionVotesRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload.patchV1RunbooksExecutionsExecutionIdVotes,
    { explode: true },
  );
  ...
  const path = pathToFunc("/v1/runbooks/executions/{execution_id}/votes")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.VotesEntity$inboundSchema),
    M.jsonErr(
