 ...
 */
export function incidentsListIncidentLinks(
  client: FirehydrantCore,
  request: operations.ListIncidentLinksRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AttachmentsLinkEntityPaginated,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListIncidentLinksRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AttachmentsLinkEntityPaginated$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
