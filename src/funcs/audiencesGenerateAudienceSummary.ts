 ...
 */
export function audiencesGenerateAudienceSummary(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GenerateAudienceSummaryRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.AIEntitiesIncidentSummaryEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
