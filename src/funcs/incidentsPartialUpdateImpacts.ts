...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PartialUpdateIncidentImpactsRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.patchV1IncidentsIncidentIdImpact, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/impact")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.IncidentEntity$inboundSchema),
    M.jsonErr(
