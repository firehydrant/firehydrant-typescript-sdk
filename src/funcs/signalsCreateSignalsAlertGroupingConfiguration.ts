 ...
 */
export function signalsCreateSignalsAlertGroupingConfiguration(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      components.CreateSignalsAlertGroupingConfiguration$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.SignalsAPIGroupingEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
