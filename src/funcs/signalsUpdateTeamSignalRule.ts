 ...
 */
export function signalsUpdateTeamSignalRule(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateTeamSignalRuleRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_team_signal_rule, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/teams/{team_id}/signal_rules/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SignalsAPIRuleEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
