 ...
 * Get initial application configuration
 */
export function accountSettingsGetBootstrap(
  client: FirehydrantCore,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.PublicApiv1BootstrapEntity,
    | FirehydrantError
...
> {
  const path = pathToFunc("/v1/bootstrap")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PublicApiv1BootstrapEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
