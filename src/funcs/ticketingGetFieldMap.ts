...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetTicketingProjectFieldMapRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    map_id: encodeSimple("map_id", payload.map_id, {
      explode: false,
      charEncoding: "percent",
    }),
    ticketing_project_id: encodeSimple(
      "ticketing_project_id",
      payload.ticketing_project_id,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.TicketingProjectFieldMapEntity$inboundSchema),
    M.jsonErr(
