 ...
 */
export async function changesCreateIdentity(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateChangeIdentityRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.postV1ChangesChangeIdIdentities, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/changes/{change_id}/identities")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.ChangeIdentityEntity$inboundSchema),
