 ...
 */
export function incidentsCreateIncidentChatMessage(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateIncidentChatMessageRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.create_incident_chat_message, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/generic_chat_messages")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.EventGenericChatMessageEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
