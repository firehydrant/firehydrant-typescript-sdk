 ...
 */
export function incidentsUpdateIncidentChatMessage(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateIncidentChatMessageRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_incident_chat_message, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.EventGenericChatMessageEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
