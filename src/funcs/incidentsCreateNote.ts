 ...
 */
export async function incidentsCreateNote(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateIncidentNoteRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.postV1IncidentsIncidentIdNotes, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/notes")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.EventNoteEntity$inboundSchema),
    M.jsonErr(
