 ...
 * Convert a task to a follow-up
 */
export async function tasksConvertToFollowup(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ConvertIncidentTaskToFollowupRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload.postV1IncidentsIncidentIdTasksTaskIdConvert,
    { explode: true },
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.TaskEntityPaginated$inboundSchema),
    M.jsonErr(
