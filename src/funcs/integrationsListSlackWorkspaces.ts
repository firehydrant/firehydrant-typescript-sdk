 ...
 */
export function integrationsListSlackWorkspaces(
  client: FirehydrantCore,
  request: operations.ListSlackWorkspacesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.IntegrationsSlackWorkspaceEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListSlackWorkspacesRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.IntegrationsSlackWorkspaceEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
