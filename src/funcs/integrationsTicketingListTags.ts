 ...
 */
export async function integrationsTicketingListTags(
  client: FirehydrantTypescriptSDKCore,
  request: operations.ListTicketTagsRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.TagEntityPaginated,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListTicketTagsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/ticketing/ticket_tags")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.TagEntityPaginated$inboundSchema),
    M.jsonErr(
