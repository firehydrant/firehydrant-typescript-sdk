 ...
 */
export function catalogEntriesCreateServiceDependency(
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CreateServiceDependency$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/service_dependencies")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.ServiceDependencyEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
