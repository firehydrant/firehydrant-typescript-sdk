 ...
 */
export function runbooksListRunbookActions(
  client: FirehydrantCore,
  request: operations.ListRunbookActionsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.RunbooksActionsEntityPaginated,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListRunbookActionsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/runbooks/actions")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.RunbooksActionsEntityPaginated$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
