 ...
 */
export function catalogEntriesCreateServiceLinks(
  client: FirehydrantCore,
  request: components.CreateServiceLinks,
  options?: RequestOptions,
): APIPromise<
  Result<
    Array<components.ServiceLinkEntity>,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CreateServiceLinks$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/services/service_links")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, z.array(components.ServiceLinkEntity$inboundSchema)),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
