 ...
 */
export function communicationUpdateStatusUpdateTemplate(
  client: FirehydrantCore,
  request: operations.UpdateStatusUpdateTemplateRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.StatusUpdateTemplateEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateStatusUpdateTemplateRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.update_status_update_template, {
    explode: true,
  });

  const pathParams = {
    status_update_template_id: encodeSimple(
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.StatusUpdateTemplateEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
