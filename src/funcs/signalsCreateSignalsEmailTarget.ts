 ...
 */
export function signalsCreateSignalsEmailTarget(
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CreateSignalsEmailTarget$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.SignalsAPIEmailTargetEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
