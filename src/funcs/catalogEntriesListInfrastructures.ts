 ...
 * Lists functionality, service and environment objects
 */
export function catalogEntriesListInfrastructures(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListInfrastructuresRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/infrastructures")();

  const query = encodeFormQuery({
    "omit_for": payload.omit_for,
    "page": payload.page,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.InfrastructureSearchEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
