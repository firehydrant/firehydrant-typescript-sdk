 ...
 */
export function metricsReportingGetSignalsMttxAnalytics(
  client: FirehydrantCore,
  request: operations.GetSignalsMttxAnalyticsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SignalsAPIAnalyticsMttxMetricsEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetSignalsMttxAnalyticsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/signals/analytics/mttx")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SignalsAPIAnalyticsMttxMetricsEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
