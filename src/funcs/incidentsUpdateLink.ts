 ...
 */
export async function incidentsUpdateLink(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateIncidentLinkRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.putV1IncidentsIncidentIdLinksLinkId, {
    explode: true,
  });
  ...
  const pathParams = {
    ...
    link_id: encodeSimple("link_id", payload.link_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v1/incidents/{incident_id}/links/{link_id}")(
    pathParams,
  );
