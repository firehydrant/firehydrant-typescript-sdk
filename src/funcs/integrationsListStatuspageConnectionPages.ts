 ...
 */
export function integrationsListStatuspageConnectionPages(
  client: FirehydrantCore,
  request: operations.ListStatuspageConnectionPagesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.IntegrationsStatuspagePageEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListStatuspageConnectionPagesRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.IntegrationsStatuspagePageEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
