 ...
 */
export function retrospectivesUpdatePostMortemQuestions(
  client: FirehydrantCore,
  request: components.UpdatePostMortemQuestions,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.PostMortemsQuestionTypeEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) => components.UpdatePostMortemQuestions$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PostMortemsQuestionTypeEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
