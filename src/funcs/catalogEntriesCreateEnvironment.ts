 ...
 */
export function catalogEntriesCreateEnvironment(
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CreateEnvironment$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.EnvironmentEntryEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
