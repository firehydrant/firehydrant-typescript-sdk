 ...
 */
export async function metricsReportingListUserInvolvementMetrics(
  client: FirehydrantTypescriptSDKCore,
  request: operations.ListUserInvolvementMetricsRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.MetricsMetricsEntity,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListUserInvolvementMetricsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/metrics/user_involvements")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.MetricsMetricsEntity$inboundSchema),
    M.jsonErr(
