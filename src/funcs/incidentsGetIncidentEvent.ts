 ...
 */
export function incidentsGetIncidentEvent(
  client: FirehydrantCore,
  request: operations.GetIncidentEventRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.IncidentEventEntity,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetIncidentEventRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/incidents/{incident_id}/events/{event_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.IncidentEventEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
