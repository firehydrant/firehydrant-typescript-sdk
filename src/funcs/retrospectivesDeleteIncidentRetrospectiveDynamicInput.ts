 ...
 */
export function retrospectivesDeleteIncidentRetrospectiveDynamicInput(
  client: FirehydrantCore,
  request: operations.DeleteIncidentRetrospectiveDynamicInputRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.IncidentsRetrospectiveFieldEntity,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.DeleteIncidentRetrospectiveDynamicInputRequest$outboundSchema
        .parse(value),
  ...
  const query = encodeFormQuery({
    "dynamic_input_field_id": payload.dynamic_input_field_id,
  });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.IncidentsRetrospectiveFieldEntity$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
