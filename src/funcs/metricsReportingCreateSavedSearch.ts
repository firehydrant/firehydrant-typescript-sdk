 ...
 */
export async function metricsReportingCreateSavedSearch(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateSavedSearchRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.postV1SavedSearchesResourceType, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/saved_searches/{resource_type}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.SavedSearchEntity$inboundSchema),
    M.jsonErr(
