 ...
 */
export async function incidentSettingsUpdateIncidentRole(
  client: FirehydrantTypescriptSDKCore,
  request: operations.UpdateIncidentRoleRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.IncidentRoleEntity,
    | errors.BadRequest
    | errors.Unauthorized
    | errors.NotFound
    | errors.Timeout
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateIncidentRoleRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.patchV1IncidentRolesIncidentRoleId, {
    explode: true,
  });

  const pathParams = {
    incident_role_id: encodeSimple(
      "incident_role_id",
      payload.incident_role_id,
      { explode: false, charEncoding: "percent" },
    ),
  };

  const path = pathToFunc("/v1/incident_roles/{incident_role_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.IncidentRoleEntity$inboundSchema),
    M.jsonErr(
