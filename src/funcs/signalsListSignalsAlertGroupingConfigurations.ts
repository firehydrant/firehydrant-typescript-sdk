 ...
 */
export function signalsListSignalsAlertGroupingConfigurations(
  client: FirehydrantCore,
  request: operations.ListSignalsAlertGroupingConfigurationsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SignalsAPIGroupingEntityPaginated,
    | FirehydrantError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListSignalsAlertGroupingConfigurationsRequest$outboundSchema
        .parse(value),
  ...
  const path = pathToFunc("/v1/signals/grouping")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SignalsAPIGroupingEntityPaginated$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
