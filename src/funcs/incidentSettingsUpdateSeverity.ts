 ...
 */
export async function incidentSettingsUpdateSeverity(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateSeverityRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.patchV1SeveritiesSeveritySlug, {
    explode: true,
  });

  const pathParams = {
    severity_slug: encodeSimple("severity_slug", payload.severity_slug, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v1/severities/{severity_slug}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.SeverityEntity$inboundSchema),
    M.jsonErr(
